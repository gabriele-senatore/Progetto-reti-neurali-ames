...
## Primo modello proviamo una lasso in cross validation e poi valutiamo
## sul test set. 

## Inizio definendo la ricetta

all_recipe <- recipe(Sale_Price ~ ., data = dat_train) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors())

## Specifico il modello e il parametro di tuning. 
## Nel mio caso il parametro di tuning è lambda. 
...
## Fisso una griglia di valori per lambda

lambda_grid <- grid_regular(
  penalty(range = c(-4, 1)),  # log10(lambda)
  levels = 30
)
...
## Tuning per la scelta di lambda
...
best_lambda <- select_best(lasso_tuned, metric = "rmse")
best_lambda

## Finalizziamo il workflow con il parametro lambda ottimale

final_wf <- finalize_workflow(wf, best_lambda)

## Stimiamo di nuovo il modello con i dati di training

final_model <- fit(final_wf, data = dat_train)
tidy(final_model)
...
pred %>% 
  ...
  labs(title = "Lasso: Predetti vs Reali, penalty = 0.00240", 
       x = "Log Prezzo Reale", y = "Log Prezzo Predetto")


## Una valutazione finale la svolgiamo rifacendo il ricampionamento dei
## dati, usando una k-fold cv ripetuta.
...
cv_results <- map_dfr(dat_folds$splits, function(split) {
  ...
  truth <- assessment(split)$Sale_Price
  
  tibble(
    rmse = rmse_vec(truth = truth, estimate = preds$.pred),
    mae  = mae_vec(truth = truth, estimate = preds$.pred),
    rsq  = rsq_vec(truth = truth, estimate = preds$.pred)
  )
})
...
v_lines <- cv_results_long %>%
  group_by(metric) %>%
  summarise(mean_val = mean(value))

ggplot(cv_results_long, aes(x = value, fill = metric)) +
  geom_histogram(bins = 12, color = "white", alpha = 0.8) +
  geom_vline(data = v_lines, aes(xintercept = mean_val), 
             color = "red", linetype = "dashed", size = 1) +
  facet_wrap(~ metric, scales = "free") +
  scale_fill_viridis_d(option = "mako", begin = 0.3, end = 0.7) + # Colori professionali
  theme_light() +
  labs(
    title = "Performance Lasso, penalty = 0.00240",
    subtitle = "La linea rossa tratteggiata indica la media",
    x = NULL, y = "Conteggio"
  )
