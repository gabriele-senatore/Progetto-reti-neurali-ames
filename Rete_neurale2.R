...
all_recipe <- formula (Lot_Frontage ~ sqrt(Lot_Area) + Neighborhood + Lot_Config+  
...
mlp_model <- mlp(
  ...
  set_engine('brulee', optimizer = 'LBFGS', mixture = 0.0, trace = FALSE) %>%
  set_mode('regression')

grid <- grid_regular(
  hidden_units(range = c(8, 30)),
  penalty(range = c(-4,-1)),
  activation(),
  levels = 5
)

folds_nn <- vfold_cv(dat_train, v=5)
...
best_params <- select_best(brulee_tuned, metric = "rmse")
best_params
...
## Lo valutiamo sul test set finale 
